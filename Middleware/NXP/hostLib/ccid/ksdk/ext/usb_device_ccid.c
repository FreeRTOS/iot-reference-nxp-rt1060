...
/* clang-format off */
uint8_t wtxCmd[] = { 0x80, 0x00, 0x00, 0x00,
                     0x00, 0x00, 0x00,         /*slots sequence No*/
                     0x80, 0x01, 0x00};
uint32_t wtxCmdLen = 10;
/* clang-format on */
/*******************************************************************************
 * API
 ******************************************************************************/
...
usb_status_t USB_DeviceSendWtxRequest(void)
{
    usb_device_ccid_struct_t *ccidHandle = &s_UsbDeviceCcidHandle[0];
    usb_status_t error = kStatus_USB_Error;
    if (!ccidHandle->bulkInBusy)
    {
        usb_device_struct_t *deviceHandle = (usb_device_struct_t *)g_UsbDeviceCcidSmartCard.deviceHandle;
        wtxCmd[6] = ccidHandle->slotsSequenceNumber[0];
        /* If the bulk IN pipe is idle, send the response data to the host */
        ccidHandle->bulkInBusy = 1U;
        uint8_t endpoint = 0x81 & USB_ENDPOINT_NUMBER_MASK;
        uint8_t direction = (0x81 & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
                                USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT;
        deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) | direction].isBusy = 0;

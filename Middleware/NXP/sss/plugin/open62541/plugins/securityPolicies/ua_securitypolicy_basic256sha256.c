...
asym_sign_sp_basic256sha256(const UA_SecurityPolicy *securityPolicy,
    ...
    if (pc->localPrivateKey.pk_info == NULL)
    {
        unsigned int outLen = 512;
        if (sss_interface_rsa_sign_data((unsigned char*)hash, UA_SHA256_LENGTH, (unsigned char*)signature->data, &outLen) != 0)
        {
            return UA_STATUSCODE_BADINTERNALERROR;
        }
        return UA_STATUSCODE_GOOD;
...
asym_getLocalSignatureSize_sp_basic256sha256(const UA_SecurityPolicy *securityPolicy,
    ...
    if (cc->policyContext->localPrivateKey.pk_info == NULL)
    {
        return sss_interface_rsa_get_key_size();
    }
    else
    {
...
asym_decrypt_sp_basic256sha256(const UA_SecurityPolicy *securityPolicy,
    ...
    if (cc->policyContext->localPrivateKey.pk_info == NULL)
    {
        ...
        unsigned int outbufLen = sizeof(outbuf);

        if (sss_interface_rsa_decrypt_data((unsigned char*)data->data, (int)data->length, (unsigned char*)outbuf, &outbufLen) != 0)
        {
            return UA_STATUSCODE_BADINTERNALERROR;
        }
        memcpy(data->data, outbuf, outbufLen);
        data->length = outbufLen;
        return UA_STATUSCODE_GOOD;
    }
    else
    {
...
policyContext_newContext_sp_basic256sha256(UA_SecurityPolicy *securityPolicy,
    ...
    if (sss_interface_is_ref_key(localPrivateKey.data, localPrivateKey.length) == 1)
    {
        UA_LOG_INFO(securityPolicy->logger, UA_LOGCATEGORY_SECURITYPOLICY, "Using Reference Key File");
    }
    else
    {
        /* Set the private key */

...
#endif /* ifndef XIP_EXTERNAL_FLASH */

/* Custom LUT Table for ISSI_IS25WPxxxA used in RT1060. */
...
static status_t flexspi_nor_flash_sector_erase( FLEXSPI_Type * base,
                                                uint32_t address )
{
    status_t status;
    flexspi_transfer_t flashXfer;
...
status_t flexspi_nor_read_data( FLEXSPI_Type * base,
...
static int32_t mflash_drv_page_program_internal( uint32_t page_addr,
                                                 uint32_t * data )
{
    uint32_t primask = __get_PRIMASK();
 ...
 */
int32_t mflash_drv_page_program( uint32_t page_addr,
                                 uint32_t * data )
{
...
/* Internal - read data */
static int32_t mflash_drv_read_internal( uint32_t addr,
                                         uint32_t * buffer,
                                         uint32_t len )
{
    uint32_t primask = __get_PRIMASK();
...
int32_t mflash_drv_read( uint32_t addr,
                         uint32_t * buffer,
                         uint32_t len )
{
    /* Check alignment */
...
void * mflash_drv_phys2log( uint32_t addr,
                            uint32_t len )
{
...
uint32_t mflash_drv_log2phys( void * ptr,
                              uint32_t len )
{
    /* take FLEXSPI remapping into account */
    uint32_t remap_offset = IOMUXC_GPR->GPR32 & 0xFFFFF000;
...
static int32_t mflash_drv_sector_update( uint32_t sector_addr,
                                         uint32_t sect_off,
                                         const uint8_t * data,
                                         uint32_t data_len )
{
...
#endif /* if !defined( MFLASH_INC_WRITES ) || !MFLASH_INC_WRITES */
...
int32_t mflash_drv_write_internal( uint32_t addr,
                                   const uint8_t * data,
                                   uint32_t data_len )
{
 ...
 * NOTE: Don't try to store constant data that are located in XIP !!
 */
int32_t mflash_drv_write( uint32_t addr,
                          const uint8_t * data,
                          uint32_t data_len )
{
